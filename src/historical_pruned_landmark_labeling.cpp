...
void historical_pruned_landmark_labeling::insert_edge(int u, int v, int t) {
    ...
    for (uint32_t iu = 0; iu < idx_u.size(); ++iu){
        label_entry_t lu = idx_u[iu];
        if (lu.v == INT_MAX) break;  // sentinel
        partial_bfs(lu.v, v, lu.d + 1, t);
    }
}
...
void historical_pruned_landmark_labeling::remove_edge(int v, int w, int t) {
    if (v < 0 || w < 0 || V <= v || V <= w) return;
    auto it = find(adj[v].begin(), adj[v].end(), (edge_t){w, t});
    if (it == adj[v].end()){ // No such edge
        return;
    }
    adj[v].erase(it);
    rebuild_index();
...
void historical_pruned_landmark_labeling::rebuild_index() {
    // Prepare
    labels.clear();
    labels.resize(V);
    rep (v, V) labels[v].push_back(((label_entry_t){INT_MAX, 0, 0}));
